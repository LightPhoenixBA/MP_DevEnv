...
public class StrideServerBase : IService
{
		...
		private NetServer netServer = new(NetConnectionConfig.GetDefaultConfig());
		public static readonly ContentManagerLoaderSettings loadSettings = new()
		{
		...
		public StrideServerBase(IServiceRegistry Services)
		{
				...
				if (sceneUrl == null)
				{
						throw new NullReferenceException($"{this} was not provided a scene to connect to from the client");
				}
		...
		public static IService NewInstance(IServiceRegistry services)
		{
				if (Instance != null)
				{
						Log.Error($"{typeof(StrideServerBase).FullName} is already declared as {Instance}");
						return Instance;
				}
				if (sceneUrl == null)
				{
