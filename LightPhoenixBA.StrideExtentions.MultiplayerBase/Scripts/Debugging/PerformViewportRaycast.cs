...
public static partial class MP_Stride_MultiplayerBaseExtentions
{
	 ...
	 public static HitResult PerformCameraRaycast(CameraComponent camera)
	 {
		...
		//	camera.Entity.Scene.Simulation.RayCast(closePoint, Vector3.Normalize(closePoint) * 100f, CollisionFilterGroups.AllFilter)
			HitResult result = StrideClientBase.Services.GetService<SceneSystem>().SceneInstance.GetProcessor<PhysicsProcessor>().Simulation.Raycast(closePoint, Vector3.Normalize(closePoint) * 100f, CollisionFilterGroups.AllFilter);//PerformServerRaycast(closePoint, farPoint);
#if DEBUG
