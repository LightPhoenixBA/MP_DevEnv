...
public static partial class MP_Stride_MultiplayerBaseExtentions
{
    ...
    public static HitResult PerformCameraRaycast(CameraComponent camera)
    {
        ...
        if (result.Succeeded)
        {
            Log.Info($"Hit at {result.Point} on {result.Collider.Entity.Scene.Name}.{result.Collider.Entity.Name}");
            DebugText.Print($"Hit: {result.Collider.Entity.Scene.Name}.{result.Collider.Entity.Name}", new Int2(mousePos), null, TimeSpan.FromSeconds(3));
